...
class CTAChecker(QualityCheck):
    ...
    def check(self, url: str, html_content: str | None = None) -> QualityCheckResult:
        ...
        total_errors = missing_required + incorrect_urls

        if total_rules == 0:
            score = 100
        else:
            score = int(max(0, ((total_rules - total_errors) / total_rules) * 100))

        # Determine status
        status = self.determine_status(score)
        if self.config.get("strict_mode", False) and validation_results['warnings']:
            status = "warning"
        ...
        if validation_results['warnings']:
            message_parts.append(f"{len(validation_results['warnings'])} warnings")
