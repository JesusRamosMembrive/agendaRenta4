    ...
    with db_cursor() as cursor:
        for name, email, notify_email, notify_browser, active in users_data:
            try:
                ...
                print(f"     Email: {email}")
                print(
                    f"     Notificaciones: Email={'✓' if notify_email else '✗'}, Browser={'✓' if notify_browser else '✗'}"
                )
...
def list_users():
    ...
    with db_cursor() as cursor:
        ...
        for user in users:
            ...
            print(
                f"           Notificaciones: Email={'✓' if user['notify_email'] else '✗'}, Browser={'✓' if user['notify_browser'] else '✗'}"
            )
            print()
...
def main():
    ...
    parser.add_argument(
        "--list", action="store_true", help="Listar usuarios existentes"
    )
