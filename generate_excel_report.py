...
def create_excel_report(crawl_run_id):
    ...
    for idx, url_row in enumerate(discovered_urls, 2):
        ...
        if url_row["last_checked"]:
            if url_row["is_broken"]:
                ...
                ws_all.cell(idx, 4).fill = PatternFill(
        ...
        # Status code
        ws_all.cell(idx, 5).value = (
            url_row["status_code"] if url_row["status_code"] else "-"
        )
        ...
        ws_all.cell(idx, 8).value = (
            url_row["discovered_at"].strftime("%d/%m/%Y %H:%M:%S")
            if url_row["discovered_at"]
            else "-"
        )
        ...
        if url_row["is_priority"]:
            ws_all.cell(idx, 3).fill = PatternFill(
    ...
    for idx, url_row in enumerate(priority_urls, 2):
        ...
        if url_row["last_checked"]:
            if url_row["is_broken"]:
                ...
                ws_priority.cell(idx, 3).fill = PatternFill(
                    start_color="FEE2E2", end_color="FEE2E2", fill_type="solid"
                )
            else:
    ...
    missing_with_names = [
        (h["url"], h["name"]) for h in hardcoded_urls if h["url"] in missing
    ]
...
def create_csv_backup(crawl_run_id):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for url_row in discovered_urls:
            discovered_at = (
                url_row["discovered_at"].strftime("%Y-%m-%d %H:%M:%S")
                if url_row["discovered_at"]
                else "-"
            )
            parent = url_row["parent_url"] or "-"
            f.write(
                f'"{url_row["url"]}",{url_row["depth"]},{discovered_at},"{parent}"\n'
            )
